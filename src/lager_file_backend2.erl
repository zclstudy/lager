...
-define(TEST_FORMATE(Formatter,Args),(error_logger:error_msg(Formatter,Args))).
...
init(LogFileConfig) when is_list(LogFileConfig) ->
    ...
    case validate_logfile_proplist(LogFileConfig) of
        ...
        Config ->
            ...
            BaseName = lager_util:expand_path(RelName),
            %%构造以日期为后缀的文件名
            Name = make_log_file(BaseName),
            %schedule_rotation(BaseName, Date)
            %%添加0点刷新文件,刷新的文件按照配置中的文件名
            schedule_change(BaseName),
            ...
            State0 = #state{name=Name, level=Level, size=Size, date=Date, count=Count, shaper=Shaper, formatter=Formatter,
                ...
                check_interval=CheckInterval,baseName=BaseName},
...
%% @private
handle_info({rotate, BaseName}, #state{baseName=BaseName,shaper=Shaper} = State) ->
    %%取消rotate调用
    % _ = lager_util:rotate_logfile(File, Count),
    %%close_file会关闭File然后设置fd=undefined
    NewFileName = make_log_file(BaseName),
    State1 = close_file(State),
    % schedule_rotation(NewFileName, Date),
    schedule_change(NewFileName),
    {ok, State1#state{name=NewFileName,shaper=Shaper#lager_shaper{id=NewFileName}}};
handle_info({shaper_expired, BaseName}, #state{shaper=Shaper, baseName=BaseName, formatter=Formatter, formatter_config=FormatConfig} = State) ->
    Report = io_lib:format(
               "lager_file_backend dropped ~p messages in the last second that exceeded the limit of ~p messages/sec",
...
handle_info(Info,State)->
    ?TEST_FORMATE("undefined Info:~p State:~p~n",[Info,State]),
    {ok,State}.
...
write(#state{name=Name, fd=FD, inode=Inode, flap=Flap} = State, Timestamp, Level, Msg) ->
    ...
    case LastCheck >= State#state.check_interval orelse FD == undefined of
        true ->
            ...
            case lager_util:ensure_logfile(Name, FD, Inode, {State#state.sync_size, State#state.sync_interval}) of
                %%注释掉对文件大小与RotSize的比较，阻止对日志文件进行rotate操作
...
%%每天的0点刷新
schedule_change(Name) ->
    RotationInterval =  common_util:datetime_to_unixtime({erlang:date(),{0,0,0}})+86400 - common_util:nowsec(),
    erlang:send_after(RotationInterval*1000,self(),{rotate,Name}),
    ok.
