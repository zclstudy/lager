...
start_link(Filename, MaxBytes, Size, Date, Count) ->
    gen_server:start_link({local, lager_crash_log}, ?MODULE, [Filename, MaxBytes,
            Size, Date, Count], []).
...
%% @private
handle_info(rotate, #state{baseName=BaseName} = State) ->
    %%取消rotate调用
...
schedule_change() ->
    ...
    erlang:send_after(RotationInterval*1000,self(),rotate),
    ok.
